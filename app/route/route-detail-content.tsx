...
// A single page (not a dynamic [id] segment) — the route's id is read
// from ?id= at runtime instead of a path param, since a saved route only
// exists in the visiting browser's own localStorage (lib/store.ts), not
// anywhere a server could look it up to render a specific id. Rendered
// by the server app/route/page.tsx, which owns this route's metadata.
