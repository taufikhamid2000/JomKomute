...
// Plots a saved route's stations as a connected line on a Leaflet map.
// Same "use client" + dynamic-import(ssr:false) pattern as
// components/report-map.tsx, for the same reason: react-leaflet touches
// window/document at module load, which would break next.config.ts's
// static export otherwise.
...
// Every station a leg passes through, in order — not just its two
// endpoints — so the plotted line follows the line's actual path instead
// of a straight endpoint-to-endpoint jump.
...
export function RouteMap({ legs }: { legs: RouteLeg[] }) {
  ...
  if (allPoints.length === 0) {
    return (
      <div className="flex h-full w-full items-center justify-center p-4 text-center text-xs text-foreground/50">
    ...
    <MapContainer center={center} zoom={DEFAULT_ZOOM} scrollWheelZoom={false} className="h-full w-full">
      ...
      {segments.map((segment, si) => {
        ...
        return segment.points.map((p, pi) => (
          <Marker key={`${si}-${pi}`} position={p.coord} icon={icon} />
        ));
      })}
    </MapContainer>
  );
